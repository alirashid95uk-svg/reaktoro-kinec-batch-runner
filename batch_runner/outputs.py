...
def write_outputs(case: ResolvedCase, result: SimulationResult) -> Path:
    ...
    if outputs.summaries.workflow_comparison:
        ...
        written.append(path)
    if outputs.summaries.secondary_mineral_assemblage:
    ...
    if outputs.solver_history.enabled:
        ...
        write_csv(path, SOLVER_HISTORY_COLUMNS, result.solver_history)
        written.append(path)
